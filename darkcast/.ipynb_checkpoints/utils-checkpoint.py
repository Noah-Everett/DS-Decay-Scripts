...
import os, sys, importlib, inspect, operator, collections, math

###############################################################################
def envpaths(var, rel = ""):
    """
    Return the environment paths for DARKCAST.
    
    var: name of the environment variable to find of the form
         DARKCAST_<var>_PATH.
    rel: relative path to the DARKCAST directory to use.
    """
    paths = []
    var = os.getenv("DARKCAST_" + var + "_PATH")
    if var: paths += [os.path.abspath(os.path.expanduser(
            os.path.expandvars(path))) for path in reversed(var.split(":"))]
    paths.append(os.path.join(os.path.dirname(os.path.abspath(
        inspect.getfile(inspect.currentframe()))), rel))
    return paths

###############################################################################
def envimport(name, paths):
    """
    Import a module, given a list of paths to search for the module.

    name:  name of the module to import.
    paths: paths to search for the module.
    """
    sys.path = [os.path.expanduser(os.path.expandvars(path))
...
class Datasets(collections.OrderedDict):
    ...
    def plots(self, lower = "lower", upper = "upper", ymax = 1e5):
        ...
        lower: label used to access the lower limit.
        upper: label used to access the upper limit.
        ymax:  maximum y value to use.
        """
        points = []
    ...
    ###########################################################################
    def ymin(self, lower = "lower", upper = "upper", xmin = -float("inf"),
            xmax = float("inf")):
        """
        Return the global y minimum for these 'Dataset's in the form (x, y).

        lower: label used to access the lower limit.
        upper: label used to access the upper limit.
        xmin:  only consider y values with x values above this.
        xmax:  only consider y values with x values below this.
        """
        points = self.plots(lower, upper)
        xval, yval = None, float("inf")
        for xs, ys in points:
            lxs, lys = [], []
            for x, y in zip(xs, ys):
                if xmin < x < xmax: lxs += [x]; lys += [y]
            if len(lxs):
                ynow = min(lys)
                if ynow < yval: xval, yval = lxs[lys.index(ynow)], ynow
        return xval, yval
...
def latex(line):
    ...
    try:
        ...
        line = prod.name
        if line == "undefined":
            if len(prod.channels) > 4: line = "$pp$"
            else: line = prod.channels[-1]
        elif line.endswith("_brem"): line = "$%s$-brem" % line[0:-5]
        elif line.endswith("_scat"): line = "$%s$-scat" % line[0:-5]
